/* 
    ...
    Approach: We'll use bisection method to find the root of the equation 2*x*cos(2x) − (x + 1)^2 = 0 on the intervals {-3,-2} and {−1,0}
*/

#include<math.h>
#include<stdio.h>

// Implementing the function 2*x*cos(2x) − (x + 1)^2 
double f(double x){
...
int main(){
    double interval1[2] = {-3,-2};
    double interval2[2] = {-1,0};
    double tol = pow(10,-5);

    double x1 = bisectionMethod(interval1,tol,30,f);
    printf("\nSolution for the equation exp(x) - x^2 + 3x - 2 = 0 on {-3,-2}: %.5f\n",x1);

    double x2 = bisectionMethod(interval2,tol,30,f);
    printf("\nSolution for the equation exp(x) - x^2 + 3x - 2 = 0 on {-1,0}: %.5f\n",x2);
    return 0;
}