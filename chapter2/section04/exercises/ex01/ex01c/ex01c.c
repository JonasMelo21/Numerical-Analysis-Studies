/*
    ...
    The question title tells us to use Newton Method to find the solution for
        x^3 - 3*x^2 * (2^-x ) + 3x*(4^-x ) - 8^-x = 0, 
    For 0 ≤ x ≤ 1 with 5 digits of accuracy. 
...
//  Function
double f(double x){
    return pow(x,3) -3*pow(x,2) * pow(2,-x) + 3* x * pow(4,-x) - pow(8,-x);
}
...
double secondDerivative(double x) {
    return 6 * x 
           - 6 * pow(2, -x) 
           + 6 * x * log(2) * pow(2, -x) 
           - 3 * pow(x, 2) * pow(2, -x) * pow(log(2), 2) 
           - 3 * log(4) * pow(4, -x) 
           + 3 * x * pow(4, -x) * pow(log(4), 2) 
           - pow(8, -x) * pow(log(8), 2);
}
...
double modifiedNewton(double p0, double tolerance, int maxIterations) {
    ...
    while (i <= maxIterations) {
        ...
        if (fabs(p - a0) < tolerance || fabs(f(p)) < tolerance) {
            printf("\nModified Newton Method executed successfully after %d iterations for  x^3 - 3*x^2 * (2^-x ) + 3x*(4^-x ) - 8^-x = 0 with p0 = %.5f.\n", i, p0);
            return p;
        }
...
int main(){
    ...
    /*Output:
        Newton Method Success (26 iterations): p0 = 0.00000
        Newton Method Solution: 0.64117

        Secant Method Success (2 iterations): p0 = 0.00000, p1 = 1.00000
        Secant Method Solution: 0.88889

        False Position Success (1 iterations): p0 = 0.00000, p1 = 1.00000
        False Position Solution: 0.88889

        Bisection Method Success (18 iterations): Interval [0.00000, 1.00000]
        Bisection Solution: 0.64119

        Modified Newton Method executed successfully after 4 iterations for  x^3 - 3*x^2 * (2^-x ) + 3x*(4^-x ) - 8^-x = 0 with p0 = 0.00000.
