/*
    ...
    to find solutions 2*x*cos(2x) − (x − 2)^2 = 0
    for  2 ≤ x ≤ 3 and 3 ≤ x ≤ 4
*/
...
int main() {
    ...
    double p0 = 2;        // Start of the interval
    double p1 = 3;        // End of the interval
    double solution = falsePosition(p0, p1, tol, max);

    // Check if the solution was found and display it
    if (solution != -1) {
        printf("\nSolution for x^3 + 3x^2 - 1 = 0 within [2, 3] is %lf", solution);
    } else {
    ...
    // Next interval [3,4]
    p0 = 3;        // Start of the interval
    p1 = 4;        // End of the interval
    double solution2 = falsePosition(p0, p1, tol, max);

    // Check if the solution was found and display it
    if (solution2 != -1) {
        printf("\nSolution for x^3 + 3x^2 - 1 = 0 within [3, 4] is %lf", solution2);
    } else {
        // Print failure message if the method did not converge
